...
??? tip "Dica!"
    Você pode contribuir com novas escalas usando a notação inteira: [Escalas wikipedia](https://en.wikipedia.org/wiki/List_of_musical_scales_and_modes).

    Todos os Pull Requests serão bem vindos! :heart:

# Notas

As notas estão sendo definidas em uma constante `NOTAS`. Foi optado por manter somente as notas no formato Natural e o Sustenido (#) para a simplificação do fluxo de trabalho.
Embora não seja totalmente correto. Para ver as 12 notas você pode:
...
NOTAS: list[LiteralString] = 'C C# D D# E F F# G G# A A# B'.split()
